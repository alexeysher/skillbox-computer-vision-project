...
Изображения обрабатываются порциями (англ. *batch*). Размер порции опередяется настроечным параметром `batch_size`. Для увеличения скорости выполнения обработки изображения предварительно подгружаются в буфер, размер которого задается с помощью параметра `buffer_size`.
